...
const AllInterested = () => {
    ...
    const removeInterested = async (username, token) => {
        try {
            const encodedUrl = encodeURIComponent(username);
            const response = await fetch(`${API}/v1/club/interestedcoach/search?username=${encodedUrl}`, {
                method: 'DELETE',
                headers: {
    ...
    const getallClubUsres = async (page, token) => {
        try {
            const response = await fetch(`${API}/allinterestedcoaches?page=${page}`, {
                method: 'GET', headers: {
                    Accept: 'application/json',
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}`
    ...
    const fetchData = async () => {
        try {
            const data = await getallClubUsres(currentPage, token); setClubUsers(data.data || []); setuserscount(data.totalInterestedCoaches || [])
        } catch (error) { console.error('Error fetching images:', error); }
    };

    const handledelete = async (username) => {
        try {
            removeInterested(username, token);
        ...
        <AdminDashLayout>
            {isAuth().role === "admin" && (
                <>
                    ...
                    <div className={styles.center}>    {userscount ? (<h3>Total &nbsp; Interested &nbsp; - &nbsp; <span> {userscount} </span></h3>) : (<></>)}</div>
