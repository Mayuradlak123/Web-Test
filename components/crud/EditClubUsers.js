...
import { singleClubUser, updateClubUser } from "@/actions/auth";
import { withRouter } from 'next/router';
import { getCookie } from "../../actions/auth";
const token = getCookie('token');

const Editclubusers = ({ router }) => {
    ...
    const [values, setValues] = useState({
     ...
     const { name, username, email, city, phonenumber, password, error, loading, message} = values;
    ...
    const handleSubmit = e => {
        ...
        setValues({ ...values, loading: true, error: false });
         ...
         const user = { name, password, phonenumber, email, username, city };

        updateClubUser(user, token, username).then(data => {
            console.log(data);
            if (data && data.error) {
    ...
    const initUser = (res) => {
        if (router.query.username) {
            singleClubUser(router.query.username).then(data => {
                if (data.error) {
                    setErrorCode(404);
                } else {
                    setValues({ ...values, name: data.name, username: data.username, email:data.email, phonenumber: data.phonenumber, city: data.city });
                }
            });
        }
    };
...
export default withRouter(Editclubusers);